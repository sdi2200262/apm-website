...
**`build/processors/placeholders.js`** resolves template placeholders to target-specific values at build time. Templates are authored once with placeholders like `{RULES_FILE}`, `{SKILL_PATH:name}`, `{ARGS}`, and `{SUBAGENT_GUIDANCE}`, and the build system replaces them per target. For example, `{RULES_FILE}` becomes `CLAUDE.md` for Claude Code but `ANTIGRAVITY.md` for Antigravity, and `{ARGS}` becomes `$ARGUMENTS` for Markdown platforms but `{{args}}` for TOML. This is what makes APM's templates platform-agnostic at the source level.
