...
Platform-specific directories: `.claude/` for Claude Code, `.github/` for GitHub Copilot, `.antigravity/` for Antigravity CLI, `.opencode/` for OpenCode, `.codex/` and `.agents/` for Codex (guides and agents go to `.codex/`; commands and skills go to `.agents/skills/`).
